...
import { registerCalendarFence } from '../src/fence'
  ...
  it('owns and disposes its stylesheet in every theme', () => {
    for (const theme of ['dark', 'light', 'reading']) {
      ...
      for (let pass = 0; pass < 2; pass++) {
        ...
        expect(document.querySelectorAll('#notes-calendar-styles')).toHaveLength(1)
        dispose()
        expect(document.querySelectorAll('#notes-calendar-styles')).toHaveLength(0)
      }
    }
  })
})

it('disposes the lazily mounted fence stylesheet', () => {
  const mock = createMockValleyApi({ manifest: declared })
  initRuntime(mock.api)
  const dispose = registerCalendarFence()
  expect(document.getElementById('notes-calendar-fence-styles')).toBeNull()
  mock.codeBlockRenderers.get('calendar')?.('', document.createElement('div'), { path: null, meta: null })
  expect(document.querySelectorAll('#notes-calendar-fence-styles')).toHaveLength(1)
  dispose()
  expect(document.getElementById('notes-calendar-fence-styles')).toBeNull()
})
...
it('keeps package styles tokenized, scalable, and consistent with SDK drop markers', () => {
  ...
  hexes.delete('#3b82f6')
  const fixedGlyph = new RegExp('notedate-(?:swatch-none|caret)')
